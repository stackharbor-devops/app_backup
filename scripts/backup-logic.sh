...
function backup(){
    ...
    echo $(date) ${ENV_NAME} "Saving data to ${DUMP_NAME} snapshot" | tee -a ${BACKUP_LOG_FILE}
    { RESTIC_PASSWORD=${ENV_NAME} restic -q -r /opt/backup backup --tag "${DUMP_NAME} ${BACKUP_ADDON_COMMIT_ID} ${BACKUP_TYPE}" ${APP_PATH} | tee -a $BACKUP_LOG_FILE; } || { echo "Backup rotation failed."; exit 1; }
